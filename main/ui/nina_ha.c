 ...
 * owns only the HA-specific overlay-state decisions (reads cfg->ha_base_url and
 * runs the shared page_conn_eval over the poller's connection counters) and the
 * tile-grid handle. ha_client.c (the entity/attr resolver) is a separate module.
 *
...
void ha_page_update(const ha_data_t *data) {
    ...
    /* Overlay states. Page-specific: reads cfg->ha_base_url and the poller's
     * connection counters, so it stays in the adapter. */
    ...
    page_conn_t st = page_conn_eval(data->ever_ok, data->connected,
                                    (int)data->fail_count);
    ...
    /* OK or STALE: render the tiles, dimmed when stale (values are the last
     * good ones -- ha_client_poll does not blank them on a failed poll). */
