...
static void apply_theme_to_page(dashboard_page_t *p) {
    ...
    if (p->lbl_filter_total_count) lv_obj_set_style_text_color(p->lbl_filter_total_count, lv_color_hex(app_config_apply_brightness(current_theme->label_color, gb)), 0);
    if (p->lbl_filter_total_text) lv_obj_set_style_text_color(p->lbl_filter_total_text, lv_color_hex(app_config_apply_brightness(current_theme->label_color, gb)), 0);
...
static void create_dashboard_page(dashboard_page_t *p, lv_obj_t *parent, int page_index) {
    ...
    lv_obj_set_flex_flow(p->row_filter_total, LV_FLEX_FLOW_ROW);
    lv_obj_set_flex_align(p->row_filter_total, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_END, LV_FLEX_ALIGN_CENTER);
    lv_obj_set_style_pad_column(p->row_filter_total, 5, 0);
    lv_obj_add_flag(p->row_filter_total, LV_OBJ_FLAG_HIDDEN);

    p->lbl_filter_total_count = lv_label_create(p->row_filter_total);
    lv_obj_set_style_text_color(p->lbl_filter_total_count, lv_color_hex(current_theme->filter_text_color), 0);
    lv_obj_set_style_text_font(p->lbl_filter_total_count, &lv_font_montserrat_24, 0);
    lv_label_set_text(p->lbl_filter_total_count, "0");

    p->lbl_filter_total_text = lv_label_create(p->row_filter_total);
    lv_obj_set_style_text_color(p->lbl_filter_total_text, lv_color_hex(current_theme->label_color), 0);
    lv_obj_set_style_text_font(p->lbl_filter_total_text, &lv_font_montserrat_20, 0);
    lv_label_set_text(p->lbl_filter_total_text, "Done");
