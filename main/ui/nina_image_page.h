 ...
 * @brief Image page spine: ONE implementation, FOUR instances (GOES, Moon,
 *        Solar, Custom URL). Each instance is a first-class page with its own
 *        runtime index (PAGE_IDX_IMG_*), LVGL page, decoded frame, poller and
 *        gates. Rendering lives in ui/nina_image_page.c; the pollers live in
 *        image_page_poll.c and are built on the poll_task spine.
 *
 ...
 *     instantly). Cap: at most IMAGE_PAGE_MAX_RESIDENT (3) frames across the four
 ...
 *   - At most one download+decode is in flight across the four pollers
...
typedef struct image_page {
    ...
    const char       *name;            /* task/log name: "img_goes" .. "img_custom" */
...
#define IMAGE_PAGE_MAX_RESIDENT 3   /* decoded frames resident across the four instances */

/* ── Instances ── */
image_page_t *image_page_get(image_src_t src);
image_page_t *image_page_by_page_idx(int page_idx);          /* NULL if not an image page */

/* Create the four instances' identities + mutexes (spawn_pollers=false) or
...
void     image_page_label(image_page_t *p, char *out, size_t sz);   /* region name / "Moon" / band label / "Custom" */
...
void image_page_set_error(image_page_t *p, const char *msg);   /* stores error_msg; Custom renders it as the caption */
