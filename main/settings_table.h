...
#define SETTINGS_TABLE(BOOL, INT, INT_RESET, FLT, FLT_RESET, ENUM, STR, STR_RESET) \
    ...
    INT_RESET (radar_map_style,              "radar_map_style",              1,     0,     2)     /* v65: which map the radar echoes are drawn over: 0 = standard NWS picture with roads and city names, 1 = state lines only (the default), 2 = state and county lines. US areas only; the worldwide source is picked in the Radar Area select (radar_token "WORLD"). RESET, not clamp: an unknown value falls back to state lines only rather than the nearest bound */ \
    INT_RESET (radar_zoom,                   "radar_zoom",                   6,     4,     7)     /* v81: how much of the world the worldwide radar picture covers, 4 (widest) .. 7 (closest); 6 is about 650 km across at mid latitudes. RESET: an unknown value falls back to the default, never to a bound. Ignored by the three US map styles */ \
    INT_RESET (radar_palette,                "radar_palette",                6,     1,     8)     /* v81: RainViewer colour scheme id. The range is 1..8 but only 1, 2, 4, 6 and 8 exist; validate_config() re-checks the exact set and resets anything else to 6 */ \
