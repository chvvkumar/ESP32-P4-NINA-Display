...
#define SETTINGS_TABLE(BOOL, INT, INT_RESET, FLT, FLT_RESET, ENUM, STR, STR_RESET) \
    ...
    INT_RESET (octoprint_layout,             "octoprint_layout",             0,     0,     4)     /* 0=bento 2=glass; 1, 3 and 4 are retired, render Bento and stay reserved (legal in NVS). RESET, not clamp: layouts are unordered names, so a stale/unknown index means "no opinion" -> bento, not "the highest layout" */ \
