...
#define SETTINGS_TABLE(BOOL, INT, INT_RESET, FLT, FLT_RESET, ENUM, STR, STR_RESET) \
    ...
    INT_RESET (octoprint_layout,             "octoprint_layout",             0,     0,     4)     /* 0=bento 1=retired (renders Bento; reserved) 2=glass 3=typographic 4=timeline. RESET, not clamp: layouts are unordered names, so a stale/unknown index means "no opinion" -> bento, not "the highest layout" */ \
